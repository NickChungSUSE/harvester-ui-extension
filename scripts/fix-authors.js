...
function extractPRNumbers(changelogContent) {
  ...
  // Match patterns like [#422] or [#123] in the changelog
  const prRegex = /\[#(\d+)\]/g;
  let match;
...
// Replace changelog entries with author information
async function replaceWithAuthors(changelogContent) {
  ...
  for (const prNumber of prNumbers) {
    ...
    // Replace the pattern: [PR_LINK] ([COMMIT_HASH]) with author info
    const pattern = new RegExp(`(\\[#${ prNumber }\\]\\([^)]+\\))\\s*\\(\\[[a-f0-9]{7,8}\\]\\([^)]+\\)\\)`, 'g');

    updatedContent = updatedContent.replace(pattern, `$1 - Author: ${ originalAuthor }`);
  }
...
async function fixAuthors() {
  ...
  // Update the changelog with author information
  changelogContent = await replaceWithAuthors(changelogContent);
