...
function restoreCommitPrefixes(changelogContent) {
  ...
  for (const [sectionTitle, prefix] of Object.entries(sectionPrefixMap)) {
    // Find the section and add prefixes to all entries in that section
    // Handle plain text format (no ### headers, just section titles)
    const sectionRegex = new RegExp(`(${ sectionTitle }\\n)((?:[^\\n]*\\n?)*?)(?=\\n[A-Z][a-zA-Z ]*\\n|$)`, 'g');

    updatedContent = updatedContent.replace(sectionRegex, (match, sectionHeader, entries) => {
      // Split entries by lines and add prefix to each non-empty line
      ...
      const prefixedLines = lines.map((line) => {
        ...
        if (trimmedLine && !trimmedLine.match(/^[A-Z][a-z]/)) {
          ...
          if (trimmedLine.startsWith('* ')) {
            return `* ${ prefix }: ${ trimmedLine.substring(2) }`;
          }
      ...
      return sectionHeader + prefixedLines.join('\\n');
    });
  }
